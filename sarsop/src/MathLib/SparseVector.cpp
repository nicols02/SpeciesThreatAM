...
main_loop_done:
  ...
  //@added byt Jonathan on 01/05/2020
    bool SparseVector::write_vector_to_file(std::ofstream& out) const{
	...
	/*	for(std::vector<SparseVector_Entry>::const_iterator iter = data.begin(); iter != data.end(); iter++){
    	...
    	int belief_size = logicalSize;
    	std::vector<double> belief(logicalSize, 0.0);

    	for(std::vector<SparseVector_Entry>::const_iterator iter = data.begin(); iter != data.end(); iter++){
	    	belief[iter->index] = iter->value;
		}
